...
func (d *local_drive) isSamsungSSD() bool {
	return d.Model == "SAMSUNG MZ7LM1T9HMJP-00005" || d.Model == "SAMSUNG MZ7LM3T8HMLP-00005"
}
...
func main() {
	...
	for _, ssd := range ssds {
		...
		if len(ssd.Errors) == 0 {
			fmt.Printf("SSD %2d (0 based, S/N: %s) - Status: OK\n", ssd.Slot, ssd.Serial)
		} else {
			errs, _ := json.Marshal(ssd.Errors)
			fmt.Printf("SSD %2d (0 based, S/N: %s) - Status: FAILED.\n\tErrors: %s.\n\n",
				   ssd.Slot, ssd.Serial, errs)
		}
		
		update_wear_log(ssd)
	}
	fmt.Println("Total SSD's detected:", len(ssds))
}
